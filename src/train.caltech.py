...
model_path = os.path.join(root_path,'trained_models/VGG/caltech')
...
data = Dataset()
trainset, _ = data.getDataset("caltech")
print("Loaded caltech Dataset: {}".format(len(trainset)))

learning_rate = tf.placeholder( tf.float32, [])
images_tf = tf.placeholder( tf.float32, [None, 224, 224, 3], name="images")
labels_tf = tf.placeholder( tf.int64, [None], name='labels')

detector = Detector(weight_path, 257)
...
for epoch in range(n_epochs):
    ...
    for start, end in zip(
        ...
        print("Batch:{}".format(start/batch_size))
        current_data = trainset[start:end]
        current_image_paths = current_data['file'].values
